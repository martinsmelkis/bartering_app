...
class InitializeCubit extends Cubit<InitializeState> {
  ...
  Future<void> startInitialization() async {
    try {
      ...
      await _userRepository.init().timeout(const Duration(seconds: 5));
      final keyWordsFromStorage = await _userRepository
          .getProfileKeywordDataMap()
          .timeout(const Duration(seconds: 5));

      if (keyWordsFromStorage != null) {
        logDebug(
          '@@@@@@@@@ Initialization complete: User ${_userRepository.userId} is authenticated.',
        );
        ...
        logDebug(
          '@@@@@@@@@@@ No onboarding data found - routing to welcome for deferred registration.',
        );
      ...
      if (errorString.contains('key_not_found') ||
        ...
        logDebugError(
          'Keystore error detected (key not found or invalid) - resetting app state',
          e,
        );
        ...
        emit(
          InitializeError(
            'Failed to initialize application: ${e.message ?? e.code}',
          ),
        );
      }
    } catch (e) {
      ...
      if (errorString.contains('key_not_found') ||
        ...
        logDebugError(
          'Keystore error detected in wrapped exception - resetting app state',
          e,
        );
        await _handleKeystoreInvalidation();
      } else if (e is TimeoutException) {
        logDebugError(
          'Initialization timed out - routing to welcome instead of leaving splash stuck',
          e,
        );
        emit(InitializeStateUnregistered());
      } else {
        emit(
          InitializeError('Failed to initialize application: ${e.toString()}'),
        );
      }
    }
  }
  ...
  Future<void> _handleKeystoreInvalidation() async {
    try {
      ...
      logDebugError(
        'Failed to recover from keystore invalidation',
        recoveryError,
      );
      emit(
        InitializeError(

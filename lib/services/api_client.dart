...
abstract class ApiClient {
      ...
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          final isPublicAccountDeletionEndpoint =
              options.path ==
                  '/api/v1/authentication/account-deletion/request' ||
              options.path == '/api/v1/authentication/account-deletion/confirm';
          final isPublicBootstrapEndpoint =
              options.path == '/api/v1/profile-create';

          if (options.path.startsWith('/api/') &&
              !isPublicAccountDeletionEndpoint &&
              !isPublicBootstrapEndpoint) {
            // 1. Get dependencies for user ID and private key
            final userRepository = getIt<UserRepository>();
            final cryptoService =
                await CryptoService.create(); // Assumes CryptoService holds the session's private key
            ...
            final privateECKey = cryptoService.ecPrivateKeyFromString(
              privateKey,
            );
            ...
            signer.init(
              true,
              pc.PrivateKeyParameter<pc.ECPrivateKey>(privateECKey),
            );

            final challengeBytes = Uint8List.fromList(utf8.encode(challenge));
            final signature =
                signer.generateSignature(challengeBytes) as pc.ECSignature;
  ...
  Future<void> updateUserConsent(@Body() UserConsentUpdateRequest request);
  ...
  Future<List<UserProfileData>> findFavoriteProfiles(
    @Path('userId') String userId,
  );
  ...
  Future<List<int>> downloadEncryptedFile(
  ...
  Future<NotificationPreferencesResponse> dismissMatch(
    @Path('matchId') String matchId,
  );
